...
async function loadAlumnos(activos) {
    ...
    try {
        ...
        json.data.forEach(a => {
            ...
            const apellidos = [p.primer_apellido, p.segundo_apellido].filter(Boolean).join(" ") || p.apellidos || "-";
            const dojo = p.dojo?.data?.attributes?.nombre || p.dojo?.nombre || "-";
            const row = `<tr>
                ...
                <td>${p.fecha_nacimiento || '-'}</td><td>${dojo}</td><td>${p.direccion || '-'}</td>
                <td>${p.poblacion || '-'}</td><td>${p.cp || '-'}</td>
                <td class="sticky-col">
                    <button class="action-btn-icon" onclick="editarAlumno('${id}')"><i class="fa-solid fa-pen"></i></button>
                    <button class="action-btn-icon delete" onclick="cambiarEstado('${id}', ${!activos})"><i class="fa-solid ${activos?'fa-user-xmark':'fa-rotate-left'}"></i></button>
                </td></tr>`;
            tbody.innerHTML += row;
        });
    } catch { tbody.innerHTML = '<tr><td colspan="12">Error al recuperar alumnos.</td></tr>'; }
}
...
async function loadDojosSelect() {
    ...
    } catch { sel.innerHTML = '<option>Error cargando dojos</option>'; }
}
...
async function runDiagnostics() {
    ...
    o.innerHTML += '<br><a href="https://stats.uptimerobot.com/xWW61g5At6" target="_blank" class="btn-monitor-ext" style="color:#33ff00; border:1px solid #33ff00; padding:5px 10px; text-decoration:none;">VER GRÁFICOS</a>';
}

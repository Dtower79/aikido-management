...
// --- CARGA DE DATOS (REFORZADA PARA v5) ---
async function loadAlumnos(activos) {
    ...
    try {
        const res = await fetch(`${API_URL}/api/alumnos?populate=dojo&${filter}&sort=primer_apellido:asc&pagination[limit]=500`, {
            ...
            if (activos) {
                tbody.innerHTML += `<tr>
                    ...
                    <td class="sticky-col">
                        ...
                        <button class="action-btn-icon delete" onclick="confirmarBaja('${id}', '${apellidos}')"><i class="fa-solid fa-user-xmark"></i></button>
                    </td></tr>`;
            } else {
                tbody.innerHTML += `<tr>
                    ...
                    <td class="sticky-col">
                        <button class="action-btn-icon restore" onclick="confirmarReactivacion('${id}', '${apellidos}')"><i class="fa-solid fa-rotate-left"></i></button>
                        <button class="action-btn-icon delete" onclick="eliminarDefinitivo('${id}', '${apellidos}')"><i class="fa-solid fa-trash-can"></i></button>
                    </td></tr>`;
            }
        });
    } catch(e) { tbody.innerHTML = `<tr><td colspan="12" style="color:red">Error recuperando datos del servidor.</td></tr>`; }
}
...
async function loadDojosCards() {
    ...
    try {
        ...
        const json = await res.json();
        const data = json.data || []; grid.innerHTML = '';
        data.forEach(d => {
...
// --- ACCIONES ---
function confirmarBaja(id, nombre) { showModal("Confirmar Baja", `¿Quieres dar de baja a ${nombre}?`, () => ejecutarEstado(id, false)); }
function confirmarReactivacion(id, nombre) { showModal("Reactivar", `¿Dar de alta de nuevo a ${nombre}?`, () => ejecutarEstado(id, true)); }
async function ejecutarEstado(id, activo) {
    const f = activo ? null : new Date().toISOString().split('T')[0];
    await fetch(`${API_URL}/api/alumnos/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${jwtToken}` }, body: JSON.stringify({ data: { activo, fecha_baja: f } }) });
    showSection(activo ? 'alumnos' : 'bajas');
}
function eliminarDefinitivo(id, nombre) {
    showModal("AVISO CRÍTICO", `¿Borrar físicamente a ${nombre}?`, () => {
        setTimeout(() => { showModal("ÚLTIMA OPORTUNIDAD", "Esta acción borrará al usuario para siempre.", async () => {
...
async function runDiagnostics() {
    ...
    o.innerHTML += '<br><a href="https://stats.uptimerobot.com/xWW61g5At6" target="_blank" class="btn-monitor-ext" style="color:#33ff00; border:1px solid #33ff00; padding:10px 20px; text-decoration:none; display:inline-block; margin-top:20px;">VEURE GRÀFICS DETALLATS</a>';
}
async function loadDojosSelect() {
    ...
    data.forEach(d => { sel.innerHTML += `<option value="${d.documentId || d.id}">${(d.attributes || d).nombre}</option>`; });
}
async function loadCiudades() {
    const res = await fetch(`${API_URL}/api/alumnos?fields[0]=poblacion`, { headers: { 'Authorization': `Bearer ${jwtToken}` } });
    const json = await res.json(); const data = json.data || [];
    const ciu = [...new Set(data.map(a => (a.attributes?.poblacion || a.poblacion)).filter(Boolean))];
...
async function exportarPDF() {
    ...
    logoImg.onload = async function() {
        ...
        const body = (json.data || []).map(a => {
            const p = a.attributes || a;
            return [new Date(p.createdAt).toLocaleDateString(), `${(p.primer_apellido || '').toUpperCase()} ${(p.segundo_apellido || '').toUpperCase()}, ${p.nombre || ''}`, p.dni || '-', p.email || '-', p.poblacion || '-', "BARCELONA", (p.dojo?.data?.attributes?.nombre || p.dojo?.nombre || '-')];
        });
